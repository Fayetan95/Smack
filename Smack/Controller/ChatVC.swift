...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
        ...
        */
        ...
        SocketService.instance.getTypingUsers { (typingUsers) in
            ...
            for (typingUser, channel) in typingUsers {
                if typingUser != UserDataService.instanace.name && channel == channelId {
                    if names == "" {
                        names = typingUser
                    } else {
    ...
    @IBAction func messageBoxEditing(_ sender: Any) {
        ...
        if messageTxtBox.text == "" {
            isTyping = false
            sendBtn.isHidden = true
            SocketService.instance.socket.emit("stopType", UserDataService.instanace.name, channelId)
        } else {
            if isTyping == false {
                sendBtn.isHidden = false
                SocketService.instance.socket.emit("startType", UserDataService.instanace.name, channelId)
            }
            isTyping = true
        }
    }
    ...
    @IBAction func sendMsgPressed(_ sender: Any) {
        if AuthService.instance.isLoggedIn {
            ...
            SocketService.instance.addMessage(messageBoby: message, userId: UserDataService.instanace.id, channelId: channelId, completion: { (success) in
                if success {
